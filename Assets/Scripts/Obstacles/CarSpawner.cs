...
public class CarSpawner : MonoBehaviour
{
    ...
    private float _spawnDistanceToBus = 100.0f;

    //Timing
    private float _timeSinceLastSpawn = 0;
    private float _timeToSpawnMax = 4.0f;

    [SerializeField] private int carLane;
    ...
    private void SpawnCars()
    {
        ...
        carLane = Random.Range(0, Utils.CarLanes.Length);
        float laneX = Utils.CarLanes[carLane];

        //Debug.Log($"Lane index: {carLane}, X: {laneX}");

        Vector3 spawnPosition = new Vector3(laneX, 0, _busTransform.position.z + _spawnDistanceToBus);


        Vector3 halfExtents = new Vector3(1.5f, 1.5f, 55f);
